...
interface Props {
  data: CasteMaster[];
  onEdit: (caste: CasteMaster) => void;
  onDelete: (caste: CasteMaster) => void;
}

export function CasteTable({ data, onEdit, onDelete }: Props) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200">
      <table className="min-w-full divide-y divide-slate-200">
        ...
        <tbody className="divide-y divide-slate-200 bg-white text-sm">
          {data.map((row) => (
            <tr key={row.id}>
              <td className="px-4 py-3 font-medium">{row.casteName}</td>
